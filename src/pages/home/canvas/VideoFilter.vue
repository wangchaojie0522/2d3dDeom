...
<template>
    <div class="baseBox">
        <div style="padding: 20px 0;">
            <div>
                上传视频：<input type="file" @change="uploadData"  />
            </div>
        </div>
        <div style="height:calc(100% - 100px);overflow:auto;margin-bootom: 20px;">
            <div>
                <h3>原视频</h3>
                <video :src="videoSrc" ref="video1" id="video1" width="600" height="480"  controls></video>
            </div>
            <div v-for="item in filterArr" :key="item.id">
                <h3>{{ item.name }}</h3>
                <canvas :id="'filterBox' + item.id"  width="600" height="480" ></canvas>
            </div>
    ...
    setup() {
        ...
        const switchToCanvas = () => {
            ...
            filterArr.value.forEach(item => {
                ...
                ctx2.drawImage(video1.value, 0, 0, 640, 480);
                let myImageData = ctx2.getImageData(0, 0,  640, 480);
        ...
        const init = (file:any) => {
            videoSrc.value = getObjectURL(file)
            switchToCanvas()
        }
        // 黑白滤镜
